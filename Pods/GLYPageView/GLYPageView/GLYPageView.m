...
static const CGFloat   kScrollViewHeight = 44.f;
...
- (void)initalUI
{
    ...
    for (NSInteger i = 0; i < self.titlesArray.count; i++)
    {
        ...
        if (i == 0)
        {
            self.lineView = [[UIView alloc] initWithFrame:CGRectMake(self.imageLeft, kScrollViewHeight - 4.f, (self.isHaveImages ? kImageWidth : 0.f) + titleWidth, 2.0f)];
...
- (CGRect)lineFrameWithLabel:(UIView *)label
{
    ...
    if (self.isHaveImages)
    {
        lineFrame = CGRectMake(CGRectGetMinX(labelRect) - kImageWidth, CGRectGetMinY(self.lineView.frame), kImageWidth + titleWidth, 2.0f);
    }
    else
    {
